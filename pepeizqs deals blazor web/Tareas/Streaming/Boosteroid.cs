...
{
	public class Boosteroid : BackgroundService
	{
		private string id = "boosteroid";

		private readonly ILogger<Boosteroid> _logger;
		private readonly IServiceScopeFactory _factoria;
		private readonly IDecompiladores _decompilador;

		public Boosteroid(ILogger<Boosteroid> logger, IServiceScopeFactory factory, IDecompiladores decompilador)
		{
			_logger = logger;
			_factoria = factory;
				...
				if (piscinaTiendas == piscinaUsada)
				{
					...
					if (conexion.State == System.Data.ConnectionState.Open)
					{
						...
						if (sePuedeUsar == true && BaseDatos.Admin.Buscar.TiendasEnUso(TimeSpan.FromSeconds(60))?.Result.Count == 0)
						{
							try
							{
								await APIs.Boosteroid.Streaming.Buscar(conexion);
